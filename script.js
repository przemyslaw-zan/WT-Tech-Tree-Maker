...
(() => {
    ...
    document.querySelector('#techtree').addEventListener('click', (e) => {
        ...
        if (vehicle.images) $('.galleria').data('galleria').load([...vehicle.images])
        ...
        document.querySelector('.galleria-info-text').style.backgroundColor = 'RGBA(0, 0, 0, 0.75)'
    ...
    function readVehicleInput() {
        ...
        const images = []
        document.querySelector('#vehicleimagelist').querySelectorAll('li').forEach((item)=>{
    ...
    function readVehicleEditInput() {
        ...
        const images = []
        document.querySelector('#vehicleimagelistedit').querySelectorAll('li').forEach((item)=>{
            const image = item.querySelectorAll('input')[0].value
            const description = item.querySelectorAll('input')[1].value.trim()
            images.push({image,description})
        })
    ...
    function createVehicleBadge(vehicle) {
        ...
        div.innerHTML = `<table>
            <tbody>
                ...
                <tr>
                    <td id="${vehicle.id}" class="vehicleBadge type_${vehicle.type} ${branchLine} connected_${vehicle.connection}" style="position:relative; ${vehicle.images||vehicle.description?'cursor:pointer;':''}">
